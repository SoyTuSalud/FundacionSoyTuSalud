...
export class authServiceImpl implements AuthService {
  ...
  async login(
    request: RequestEntity<AuthDTO>, response: NextApiResponse
    ...
    try {
      ...
      const token = await jwt.sign({role: userDto.role, 
        correo: userDto.correo, 
        statusAccount: 
        userDto.statusAccount}, process.env.ENV_KEY_TOKEN!, {
        expiresIn: '6h',
      })
  ...
  async signIn(request: RequestEntity<AuthSignInDTO>): Promise<ResponseEntity<null>> {
    try {
  ...
  async loginAdmin(
    ...
    try {
      ...
      this.validatePass(user.contrasena, request.body.contrasena)
