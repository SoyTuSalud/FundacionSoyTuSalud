...
import ServiciosMedicosController from '../controller/serviciosMedicos.controller'
import {roleEnum} from '@common/enums/role.enum'
import {logger, loggerRequest} from "@common/logger/winston.config";
import {loggerMessage} from "@common/enums/logger.enum";


export class ServiciosMedicosRoutes {
  constructor(private serviciosMedicosController: ServiciosMedicosController,
              private router: NodeRouter<NextApiRequest, NextApiResponse>) {}

  public runRouterServiciosMedicos() {

    const methodName = "runRouterServiciosMedicos"
    ...
    this.router.post("/api/v1/pacientes", (req, res, next) => {
      return this.serviciosMedicosController.postPacientes(req, res)
    })

    //GET methods
    this.router.get("/api/v1/pacientes", (req, res, next) => {
      validateRol(req, [roleEnum.ADMIN])
      return this.serviciosMedicosController.getPacientes(req, res)
    })

    this.router.get("/api/v1/pacientes/historia", (req, res, next) => {
      validateRol(req, [roleEnum.ADMIN])
      return this.serviciosMedicosController.getPacientesTuHistoria(req, res)
    });

    this.router.get("/api/v1/pacientes/find/:id", (req, res, next) => {
      validateRol(req, [roleEnum.ADMIN])
      return this.serviciosMedicosController.getPacientesById(req, res)
    });

    //PATCH METHODS
    this.router.patch("/api/v1/pacientes", (req, res, next) => {
      validateRol(req, [roleEnum.PACIENTE, roleEnum.ADMIN])
      return this.serviciosMedicosController.patchPacientes(req, res)
    });
