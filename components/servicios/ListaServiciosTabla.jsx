...
export const ListaServiciosTabla = ({ ListaServicios, ...rest }) => {
  ...
  const [actualizarEstadoServicio, { loading }] = useMutation(
  ...
  const handleSelectAll = (event) => {
    ...
    if (event.target.checked) {
      newSelectedCustomerIds = ListaServicios.map(
        (service) => service.identificacion,
      )
    } else {
    ...
    <Card {...rest} className="mt-5">
      <TableContainer>
        <Table>
          <TableHead>
            <TableRow>
              <TableCell>Nombre completo del servicio</TableCell>
              <TableCell>Categoria</TableCell>
              <TableCell>Dias disponibles</TableCell>
              <TableCell>Horario de atención</TableCell>
              <TableCell>Direccion</TableCell>
              <TableCell>prestador del servicio</TableCell>
          ...
          <TableBody>
            {ListaServicios.slice(0, limit).map((service, index) => (
              <TableRow
                ...
                key={index}
                selected={
                  selectedCustomerIds.indexOf(service.identificacion) !== -1
                }
              >
                <TableCell>{service.especialidad}</TableCell>
                ...
                <TableCell>{service.direccionServicio}</TableCell>
                <TableCell>{service.nombreResponsable}</TableCell>
                <TableCell>{service.celularServicio}</TableCell>
                <TableCell>{service.whatsAppServicio}</TableCell>
                <TableCell>{service.valorServicio}</TableCell>
                <TableCell>{service.modalidad}</TableCell>
                <TableCell>
                  <FormControlLabel
                    control={
                      <IOSSwitch
                        sx={{ m: 1 }}
                        onChange={(e) => handleState(e, service.identificacion)}
                        defaultChecked={service.habilitado}
                      />
                    }
                  />
                </TableCell>
